  ...
  if (email && password) {
    ...
    return res
      ...
      .json({
        ...
        success: 0,
        result: "",
        message: "Username and Password Must be fill.",
      });
  }
};
...
export const generateNewToken = (req: Request, res: Response): any => {
  ...
  if (!authHeader) {
    return res
      .status(401)
      .json({ status: 401, success: 0, result: "", message: "Invalid Token" });
  }
  ...
  if (username && password) {
    ...
    return res
      ...
      .json({
        ...
        success: 1,
        result: "",
        message: username + " " + encpassword,
      });
  } else {
    return res
      .status(401)
      .json({ status: 401, success: 0, result: "", message: "No Login Data" });
  }
};
...
export const changePassword = (req: Request, res: Response): any => {
  ...
  if (!authHeader) {
    return res
      .status(401)
      .json({ status: 401, success: 0, result: "", message: "Unauthorized" });
  }
  const token = authHeader.split(" ")[1];
  const secret = process.env.JWT_SECRET || "";
  const decoded = jsonwebtoken.verify(token, secret) as any;
  ...
  if (userID && password) {
    ...
    User.findOne({ _id: userID })
      ...
      .catch((err) => {
        return res
          .status(500)
          .json({ status: 500, success: 0, result: "", message: String(err) });
      });
  } else {
    return res
      .status(422)
...
export const updateProfile = (req: Request, res: Response): any => {
  ...
  if (name && email && userID) {
    User.findOne({ email: email, _id: { $ne: userID } })
      .then((findDupplicateEmail) => {
...
export const requireLogin = expressJWT({
  ...
  onExpired: async (req: Request, err: any) => {
    if (new Date().getTime() - err.inner.expiredAt < 5000) {
      return;
    }
    throw err;
  },
...
export const verifyToken = (
  ...
  jsonwebtoken.verify(token, secret, (err: any, decoded: any) => {
    if (err) {
      ...
      res
        .status(200)
        .json({ status: 200, success: 1, result: decoded, message: "Success" });
      next();
    }
  });
};
  ...
  try {
    ...
    if (email) {
      ...
      if (returnProcess.status === 200) {
        ...
        const returnRegisterProcess = await registerProcess(
          email,
          encpassword,
          name,
          picture,
        ...
        if (returnRegisterProcess) {
          if (returnRegisterProcess.status === 200) {
            ...
            return res
              .status(returnLoginProcess.status)
              .json(returnLoginProcess);
          }
        } else {
          return res
            .status(400)
    ...
    return res
      .status(500)
      .json({ status: 500, success: 0, result: "", message: err.message });
  }
};
    ...
    try {
      ...
      if (userResult) {
        return {
          ...
          success: 1,
          result: "Success",
          message: "Insert Data Complete " + email,
        };
      } else {
        return {
          status: 400,
